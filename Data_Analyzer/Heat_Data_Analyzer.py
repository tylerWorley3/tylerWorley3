...
def calculate_cycle_durations(df, column):  
    stripped_column = df[column].str.strip()
...
def calculate_heater_stats(df, column, day_count):
    ...
    complete_cycles = df.loc[df['ON_START'].notnull() & df['ON_END'].notnull()].copy()
